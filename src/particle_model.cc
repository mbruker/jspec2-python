...
//    vector<double>& dp_p = ion_sample.cdnt(Phase::DP_P);
    if(ecool_solver) {
        ...
        apply_cooling_kick(freq, ion, ion_sample, dt);
    }
    if(ibs_solver) {
        apply_ibs_kick(ion, ion_sample, dt);
    }

//    if(edge_effect) {
//        apply_edge_kick(cooler, ebeam, ion, ion_sample);
//    }

    move_particles(ion, ion_sample);
...
void ParticleModel::apply_ibs_kick(Beam& ion, Ions& ion_sample, double dt) {
